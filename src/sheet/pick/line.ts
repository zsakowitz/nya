...
export const PICK_LINE_P1 = createPicker({
  find(_, at, sheet) {
...
const PICK_LINE_P2 = createPicker({
  ...
  draw(p1, p2, sheet) {
    if (p2) {
      drawLine([p1.val.value, p2.val.value], sheet.paper)
    }
  ...
  select(p1, p2, sheet) {
    ...
    new CmdVar("n", sheet.options).insertAt(cursor, L)
